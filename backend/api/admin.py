...
def dashboard():
    ...
    try:
        ...
        logger.exception("DB error building dashboard stats")
...
def list_users():
    ...
    try:
        ...
        logger.exception("DB error listing users")
...
def admin_list_treks():
    ...
    try:
        ...
        logger.exception("DB error listing treks for admin")
...
def update_user_status(user_id):
    ...
    try:
        ...
        db.session.commit()
        logger.info("User status updated: user_id=%s is_active=%s is_blacklisted=%s", user_id, user.is_active, user.is_blacklisted)
    except SQLAlchemyError:
        db.session.rollback()
        logger.exception("DB error updating user status: user_id=%s", user_id)
...
def create_staff():
    ...
    staff = User(username=data["username"], email=data["email"], role="staff",
                 full_name=data.get("full_name", ""), phone=data.get("phone", ""), is_active=True)
    staff.set_password(data["password"])
    try:
        db.session.add(staff)
        db.session.commit()
        logger.info("Staff account created: staff_id=%s username=%r", staff.id, staff.username)
    except SQLAlchemyError:
        db.session.rollback()
        logger.exception("DB error creating staff: username=%r", data.get("username"))
...
def list_all_bookings():
    ...
    try:
        ...
        logger.exception("DB error listing all bookings")
