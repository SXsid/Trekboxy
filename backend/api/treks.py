...
def list_treks():
    ...
    try:
        ...
        logger.exception("DB error listing treks")
        return jsonify({"error": "Failed to fetch treks"}), 500
...
def search_treks():
    try:
        ...
        logger.exception("DB error searching treks")
...
def create_trek():
    ...
    try:
        trek = Trek(
            ...
            description=data.get("description", ""), difficulty=data["difficulty"],
            duration_days=data["duration_days"], total_slots=data["total_slots"],
            available_slots=data["total_slots"], status="Pending",
            ...
            price=data.get("price", 0.0), created_by=user_id,
        )
        db.session.add(trek)
        db.session.commit()
        logger.info("Trek created: id=%s name=%r by admin_id=%s", trek.id, trek.name, user_id)
    except ValueError as e:
        logger.warning("Invalid date in create_trek: %s", e)
        return jsonify({"error": f"Invalid date format: {e}"}), 400
    except SQLAlchemyError:
        db.session.rollback()
        logger.exception("DB error creating trek: name=%r", data.get("name"))
        return jsonify({"error": "Failed to create trek"}), 500
...
def update_trek(trek_id):
    ...
    try:
        ...
        if "duration_days" in data: trek.duration_days = data["duration_days"]
        if "price" in data: trek.price = data["price"]
        if "status" in data:
            if data["status"] not in VALID_STATUSES:
                return jsonify({"error": "Invalid status"}), 400
            trek.status = data["status"]
        if "start_date" in data: trek.start_date = date.fromisoformat(data["start_date"])
        if "end_date" in data: trek.end_date = date.fromisoformat(data["end_date"])
        db.session.commit()
        logger.info("Trek updated: id=%s", trek_id)
    except ValueError as e:
        return jsonify({"error": f"Invalid value: {e}"}), 400
    except SQLAlchemyError:
        db.session.rollback()
        logger.exception("DB error updating trek: id=%s", trek_id)
...
def delete_trek(trek_id):
    ...
    try:
        ...
        db.session.commit()
        logger.info("Trek deleted: id=%s name=%r", trek_id, trek.name)
    except SQLAlchemyError:
        db.session.rollback()
        logger.exception("DB error deleting trek: id=%s", trek_id)
        return jsonify({"error": "Failed to delete trek"}), 500
    cache_delete("treks:all", f"treks:{trek_id}")
...
def assign_staff(trek_id):
    ...
    try:
        ...
        db.session.commit()
        logger.info("Staff assigned: staff_id=%s trek_id=%s", staff_id, trek_id)
    except SQLAlchemyError:
        db.session.rollback()
        logger.exception("DB error assigning staff: trek_id=%s", trek_id)
        return jsonify({"error": "Failed to assign staff"}), 500
    cache_delete("treks:all", f"treks:{trek_id}")
    return jsonify({"message": f"Staff {staff.username} assigned to trek {trek.name}"}), 200
